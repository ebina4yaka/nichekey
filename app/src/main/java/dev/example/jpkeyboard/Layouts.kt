...
data class Key(val label: String, val out: String = label)
data class Row(val keys: List<Key>)
data class Layout(val name: String, val rows: List<Row>)
...
object Layouts {
    ...
    val BUILTIN = listOf(
        Layout("QWERTY", listOf(
            row("q", "w", "e", "r", "t", "y", "u", "i", "o", "p"),
            row("a", "s", "d", "f", "g", "h", "j", "k", "l"),
            row("z", "x", "c", "v", "b", "n", "m", ",", "."),
        )),
        Layout("Eucalyn", listOf(
            row("q", "w", ",", ".", ";", "m", "r", "d", "y", "p"),
            row("a", "o", "e", "i", "u", "g", "t", "k", "s", "n"),
            row("z", "x", "c", "v", "f", "b", "h", "j", "l", "/"),
        )),
        Layout("Eucalyn改", listOf(
            row(";", ",", ".", "p", "q", "y", "g", "d", "m", "f"),
    ...
    fun parse(json: String): Layout {
        ...
        return Layout(name, (0 until rowArr.length()).map { r ->
    ...
    fun saveCustom(c: Context, json: String) {
        parse(json) // 検証してから保存
    ...
    fun currentIndex(c: Context): Int =
        prefs(c).getInt("index", 0).coerceIn(0, load(c).size - 1)

    fun setCurrentIndex(c: Context, i: Int) {
